"""Quantization utilities used by SparseGPT pruning."""
...
class Quantizer(nn.Module):
    """GPTQ-style quantizer for weight quantization during pruning."""
    ...
    def configure(self, bits, perchannel=False, sym=True, mse=False,
                  norm=2.4, grid=100, maxshrink=0.8, grouprows=1):
        ...
        if self.mse:
            ...
            for i in range(int(self.maxshrink * self.grid)):
                ...
                zero1 = (torch.round(-xmin1 / scale1)
                         if not self.sym else self.zero)
